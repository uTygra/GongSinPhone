...
1. ~~[Phase 0: PoC 스크립트](#phase-0-poc-스크립트)~~ ✅ 완료
2. ~~[Phase 1: 프로젝트 초기 세팅](#phase-1-프로젝트-초기-세팅)~~ ✅ 완료
3. ~~[Phase 1: Prisma 스키마](#phase-1-prisma-스키마)~~ ✅ 완료
4. [Phase 1: NextAuth 인증](#phase-1-nextauth-인증) ⚠️ 카카오 ✅ / 구글·로그인 페이지·미들웨어 ⬜
...
16. **[보안: 웹훅 인증 (S-1 + S-2)](#보안-웹훅-인증-s-1--s-2)** 🔴 출시 전 필수
17. **[보안: allowedApps + 기기 소유권 (S-3 + S-4)](#보안-allowedapps-서버-검증--기기-소유권-s-3--s-4)** 🔴 출시 전 필수
18. **[보안: Cron 인증 + idempotency (S-5)](#보안-cron-인증--idempotency-s-5)** 🟡 출시 전
19. **[보안: 정책 빌더 강화 (S-6 + S-7)](#보안-free-정책-강화--expired-fallback-s-6--s-7)** 🟡 출시 전

---

## Phase 0: PoC 스크립트

### ~~프롬프트 0-1: 엔터프라이즈 + 정책 생성 스크립트~~ ✅ 완료

> 완료: `scripts/poc-setup.ts` 생성 및 실행  
> 결과: 엔터프라이즈 `enterprises/LC03bg6jul` + 정책 3벌(study/free/expired) 등록 완료  
> `.env.local`에 `AMAPI_ENTERPRISE_NAME` 저장됨
...
### ~~프롬프트 0-2: QR 등록 토큰 발급 스크립트~~ ✅ 완료

> 완료: `scripts/poc-enroll.ts` 생성 및 실행  
> 결과: QR 코드 이미지(`poc-qr.png`) 생성 완료, 갤럭시 QR 스캔 테스트 진행 중
...
### ~~프롬프트 1-1: Next.js 프로젝트 세팅~~ ✅ 완료

> 완료: Next.js **16.2.9** + TypeScript + Tailwind **4** + Prisma **6** (Next.js 15 → 16으로 버전 차이)  
> `.env.local` 설정, `tsconfig.json` paths(`@/*`) 완성  
> 실제 버전: `next-auth@5.0.0-beta.31`, `@supabase/supabase-js`, `googleapis`
...
### ~~프롬프트 1-2: Prisma 스키마 작성~~ ✅ 완료

> 완료: `prisma/schema.prisma` — User, Device, Subscription, Schedule 모델 + enum 정의  
> `npx prisma db push` 완료 (Supabase에 테이블 생성됨)  
> Prisma 7 breaking change로 **Prisma 6(v6.19.3)** 사용 (`url`/`directUrl`은 schema.prisma에 유지)
...
## Phase 1: NextAuth 인증 ⚠️ 부분 완료

### 프롬프트 1-3: NextAuth v5 카카오 + 구글 로그인

> ✅ 완료: `src/lib/auth.ts` (next-auth v5 API), `src/app/api/auth/[...nextauth]/route.ts`  
> ✅ 완료: 카카오 OAuth — `/api/auth/providers`, `/api/auth/signin/kakao` 동작 확인  
> ⬜ 미완: Google OAuth 연동, `/login` 페이지 UI, `middleware.ts`, `auth-helpers.ts`
...
### 프롬프트 보안-1: Pub/Sub OIDC 검증 + 포트원 서명 검증

```
공신폰 SaaS의 웹훅 인증을 구현해줘. 두 곳 모두 반드시 구현해야 한다.

━━━ 1. AMAPI Pub/Sub 웹훅 OIDC 검증 ━━━

파일: src/lib/pubsub-auth.ts

Google Cloud Pub/Sub push 방식은 요청 헤더에
  Authorization: Bearer <OIDC JWT>
를 포함한다. 이를 반드시 검증해야 한다.

구현:
import { OAuth2Client } from 'google-auth-library'
const oauthClient = new OAuth2Client()

export async function verifyPubSubToken(authHeader: string | null): Promise<void> {
  if (!authHeader?.startsWith('Bearer ')) throw new Error('Missing auth header')
  const token = authHeader.slice(7)
  const ticket = await oauthClient.verifyIdToken({
    idToken: token,
    audience: process.env.NEXT_PUBLIC_BASE_URL + '/api/devices/webhook',
  })
  const payload = ticket.getPayload()
  // 발급자가 Google이고 email이 Pub/Sub 서비스 계정인지 확인
  if (payload?.email_verified !== true) throw new Error('Token not verified')
}

파일: src/app/api/devices/webhook/route.ts 수정
- 핸들러 최상단에서 verifyPubSubToken(req.headers.get('authorization')) 호출
- 실패 시 401 반환 (Pub/Sub이 재시도하지 않도록 400이 아닌 200 반환 고려)
- 실제로는 검증 실패도 200을 반환해서 Pub/Sub 무한 재시도를 막아야 함
  → 검증 실패는 로깅만 하고 200 반환, 이상 이벤트는 dead-letter queue로

━━━ 2. 포트원 v2 결제 웹훅 서명 검증 ━━━

파일: src/lib/portone-auth.ts

포트원 v2 웹훅 서명 검증 방식:
- 헤더: webhook-id, webhook-timestamp, webhook-signature
- 검증 input = "{webhook-id}.{webhook-timestamp}.{raw body}"
- HMAC-SHA256(PORTONE_WEBHOOK_SECRET, input) → base64
- webhook-signature 헤더는 공백으로 구분된 여러 서명 중 하나와 일치하면 OK
- webhook-timestamp가 현재 시각과 5분 이상 차이나면 거부 (replay attack 방지)
...
export function verifyPortoneWebhook(
  ...
  if (Math.abs(Date.now() / 1000 - ts) > 300) throw new Error('Expired timestamp')
  ...
  // 3. 비교 (타이밍 공격 방지: timingSafeEqual 사용)
...
파일: src/app/api/billing/webhook/route.ts 수정
- req.text()로 raw body 먼저 읽기 (req.json() 사용 금지 — body 소비됨)
- verifyPortoneWebhook() 호출 후 JSON.parse(rawBody)
- 검증 실패 시 400 반환
```

---

## 보안: allowedApps 서버 검증 + 기기 소유권 (S-3 + S-4)

### 프롬프트 보안-2: allowlist 강제 + 소유권 공통 헬퍼

```
공신폰 SaaS의 API 보안 헬퍼를 구현해줘.

━━━ 1. allowedApps 서버 allowlist ━━━

파일: src/lib/policies/allowed-apps.ts
...
export const ALWAYS_BLOCKED = new Set([
  ...
  'com.google.android.gms',             // GMS (우회 경로)
...
// 부모가 ON/OFF 가능한 앱 목록
export const PARENT_TOGGLEABLE: Record<string, string> = {
  'com.kakao.talk': '카카오톡',
  'com.nhn.android.search': '네이버 사전',
  'com.samsung.android.calendar': '캘린더',
  'com.google.android.apps.youtubemusic': '유튜브 뮤직',
  'com.iloen.melon': '멜론',
  'com.sec.android.app.popupcalculator': '계산기',
  'com.ebs.ebsmath': 'EBS Math',
  'com.naver.nanum': '네이버 전자책',
}

export function validateAllowedApps(packages: string[]): void {
  for (const pkg of packages) {
    if (ALWAYS_BLOCKED.has(pkg)) {
      throw new Error(`차단 고정 앱은 허용할 수 없습니다: ${pkg}`)
    }
    if (!PARENT_TOGGLEABLE[pkg]) {
      throw new Error(`허용 목록에 없는 앱입니다: ${pkg}`)
    }
  }
}

파일: src/app/api/devices/[id]/policy/route.ts
- allowedApps 파싱 직후 validateAllowedApps(allowedApps) 호출
- 실패 시 400 반환

━━━ 2. 기기 소유권 공통 검증 ━━━

파일: src/lib/auth-helpers.ts 에 추가
...
// 서버 컴포넌트 / API 라우트에서 세션 강제
export async function requireAuth() {
  const session = await auth()
  if (!session?.user?.id) throw new UnauthorizedError('로그인이 필요합니다')
  return session
}

// 기기 소유권 검증 — 모든 /api/devices/[id]/* 에서 반드시 호출
export async function requireDeviceOwnership(deviceId: string, userId: string) {
  ...
  if (device.userId !== userId) throw new ForbiddenError('접근 권한이 없습니다')
  return device
}

// 에러 → HTTP 응답 변환 (모든 API 라우트에서 공통 사용)
...
// 사용 패턴 (모든 device API에 적용):
// export async function PATCH(req, { params }) {
//   try {
//     const session = await requireAuth()
//     const device = await requireDeviceOwnership(params.id, session.user.id)
//     // ... 실제 로직
...
## 보안: Cron 인증 + idempotency (S-5)

### 프롬프트 보안-3: Cron 인증 강화

```
공신폰 SaaS Vercel Cron 인증과 idempotency를 구현해줘.

━━━ 1. Cron 인증 미들웨어 ━━━

파일: src/lib/cron-auth.ts

import crypto from 'crypto'

export function verifyCronSecret(authHeader: string | null): void {
  const token = authHeader?.replace('Bearer ', '')
  if (!token) throw new Error('Missing cron secret')
  // 타이밍 공격 방지
  const expected = Buffer.from(process.env.CRON_SECRET!)
  const provided = Buffer.from(token)
  if (expected.length !== provided.length) throw new Error('Invalid cron secret')
  if (!crypto.timingSafeEqual(expected, provided)) throw new Error('Invalid cron secret')
}

━━━ 2. 결제 Cron idempotency ━━━
...
export async function POST(req: Request) {
  ...
  for (const sub of subscriptions) {
    // idempotency: 이번 달 이미 결제됐으면 스킵
    if (sub.lastChargedPeriod === currentPeriod) { skipped++; continue }
    
    try {
      await issueAndCharge(sub.billingKey, getPlanAmount(sub.plan), '공신폰 월 구독')
      await prisma.subscription.update({
        where: { id: sub.id },
        data: {
          currentPeriodEnd: addDays(new Date(), 30),
          lastChargedPeriod: currentPeriod,  // 중복 방지
          failCount: 0,
        },
      })
      charged++
    } catch (err) {
      await handlePaymentFailed(sub, err)
      failed++
    }
  }

  return Response.json({ charged, skipped, failed })
}

Prisma schema에 Subscription 모델에 필드 추가:
  lastChargedPeriod String?  // "2026-06" 형식, 중복 결제 방지

━━━ 3. 스케줄 Cron idempotency ━━━

파일: src/app/api/schedule/apply/route.ts
- POST 핸들러 최상단에서 verifyCronSecret() 호출
- 각 기기별로 현재 amapiPolicyName이 이미 올바른 상태면 AMAPI 호출 스킵
  (불필요한 API 호출 줄이기)
- 실행 결과 로깅: { applied, skipped, errors, durationMs }
```

---

## 보안: free 정책 강화 + expired fallback (S-6 + S-7)

### 프롬프트 보안-4: 정책 빌더 보안 강화

```
공신폰 SaaS 정책 빌더를 보안 강화해줘.

파일: src/lib/policies/builder.ts

━━━ 공통 보안 베이스라인 (모든 정책에 적용) ━━━
...
const SECURITY_BASELINE = {
  ...
  networkResetDisabled: true,
  modifyAccountsDisabled: true,
  screenCaptureDisabled: false,  // free에서는 허용 가능
  untrustedAppsPolicy: {
    untrustedAppInstallSources: 'DISALLOW_INSTALL',
  },
  // 항상 차단 앱 (모든 정책 공통)
  // Play Store, 브라우저, 유튜브는 정책에 명시적으로 BLOCKED
}

const ALWAYS_BLOCKED_APPS = [
  { packageName: 'com.android.vending', installType: 'BLOCKED' },       // Play Store
  { packageName: 'com.sec.android.app.sbrowser', installType: 'BLOCKED' }, // 삼성 브라우저
  { packageName: 'com.android.chrome', installType: 'BLOCKED' },
  { packageName: 'com.google.android.youtube', installType: 'BLOCKED' },
]

━━━ study 정책 (기존 유지, SECURITY_BASELINE 포함) ━━━
- kioskCustomLauncherEnabled: true
- 전화, 문자, 시계 + 부모 허용 앱만 FORCE_INSTALLED
- ALWAYS_BLOCKED_APPS 포함

━━━ free 정책 (보안 강화) ━━━
- SECURITY_BASELINE 전체 유지 (factoryResetDisabled, modifyAccountsDisabled 등)
- kioskCustomLauncherEnabled: false  ← 일반 런처 복귀
- screenCaptureDisabled: false  ← 자유시간에는 스크린샷 허용
- ALWAYS_BLOCKED_APPS 유지  ← Play Store, 브라우저 차단 유지!
- 전화, 문자, 카카오톡, 자유시간 허용앱 FORCE_INSTALLED

━━━ expired 정책 (fallback 포함) ━━━

export function buildExpiredPolicy(enterpriseName: string, hasRenewalApp: boolean) {
  if (hasRenewalApp) {
    return {
      ...SECURITY_BASELINE,
      kioskCustomLauncherEnabled: true,
      applications: [
        ...
        { packageName: 'com.gongshin.renewal_notice', installType: 'FORCE_INSTALLED' },
        ...ALWAYS_BLOCKED_APPS,
      ],
    }
  } else {
    // renewal_notice 앱 미설치 시 fallback: 전화만
    return {
      ...SECURITY_BASELINE,
      kioskCustomLauncherEnabled: true,
      kioskCustomization: { /* 전화만 보이는 심플 KIOSK */ },
      applications: [
        { packageName: 'com.samsung.android.dialer', installType: 'FORCE_INSTALLED' },
        ...ALWAYS_BLOCKED_APPS,
      ],
...
파일: src/lib/amapi.ts 수정
- expired 정책 적용 전 getDeviceInfo()로 앱 설치 목록 확인
- hasRenewalApp 여부에 따라 buildExpiredPolicy(name, hasRenewalApp) 선택
```
...
| 프롬프트 | 항목 | 상태 |
|---|---|---|
| 0-1 | 엔터프라이즈 + 정책 생성 스크립트 | ✅ 완료 |
| 0-2 | QR 등록 토큰 발급 스크립트 | ✅ 완료 |
| 1-1 | Next.js 프로젝트 세팅 | ✅ 완료 (v16.2.9) |
| 1-2 | Prisma 스키마 + DB push | ✅ 완료 |
| 1-3 | NextAuth 카카오 로그인 | ✅ 완료 |
| 1-3 | Google OAuth + 로그인 페이지 + 미들웨어 | ⬜ 미완 |
| 1-4 | AMAPI 연동 라이브러리 (`lib/amapi.ts`) | ⬜ 미완 |
| 1-5 | 기기 등록 API + Pub/Sub 웹훅 | ⬜ 미완 |
| 1-6 | 정책 변경 API | ⬜ 미완 |
| 1-7 | 대시보드 메인 화면 | ⬜ 미완 |
| 1-8 | 앱 관리 화면 | ⬜ 미완 |
| 1-9 | 시간표 설정 화면 | ⬜ 미완 |
| 1-10 | QR 온보딩 페이지 | ⬜ 미완 |
| 1-11 | 포트원 v2 결제 연동 | ⬜ 미완 |
| 1-12 | 스케줄 Cron | ⬜ 미완 |
| 1-13 | Pub/Sub 웹훅 | ⬜ 미완 |
| 2-1 | 위치 추적 (Phase 2) | ⬜ 미완 |
| 보안-1 | Pub/Sub OIDC 검증 + 포트원 서명 검증 | 🔴 미완 — 출시 전 필수 |
| 보안-2 | allowedApps allowlist + 기기 소유권 공통 헬퍼 | 🔴 미완 — 출시 전 필수 |
| 보안-3 | Cron 인증 강화 + idempotency | 🟡 미완 |
| 보안-4 | free/expired 정책 빌더 보안 강화 | 🟡 미완 |
